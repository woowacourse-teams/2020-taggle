...
public class TagBookmarkAcceptanceTest extends AcceptanceTest {
    ...
    void manageCategory() {
        ...
        // 해당 태그의 북마크를 가져온다,
        tagBookmarkResponse = findBookmarksOfTag(tagResponse.getId());

        assertThat(tagBookmarkResponse.getName()).isEqualTo("taggle");
        assertThat(tagBookmarkResponse.getBookmarks()).hasSize(1);

        // 해당 북마크의 태그를 가져온다.
        BookmarkTagResponse bookmarkTagResponse = findTagsOfBookmark(bookmarkResponse.getId());
    ...
    public TagBookmarkResponse findBookmarksOfUntagged() {
    ...
    public BookmarkTagResponse findTagsOfBookmark(final Long id) {
